...
from fast_odoo_mcp.access_control import (
    AccessControlError,
    AccessController,
)
...
class TestWriteAllowlist:
    """Test ODOO_MCP_WRITE_ALLOWLIST functionality."""

    def _make_controller(self, write_allowlist=None, model_allowlist=None, model_blocklist=None):
        config = OdooConfig(
            url="http://localhost:8069",
            api_key="test-key",
            readonly=False,
            write_allowlist=write_allowlist or frozenset(),
            model_allowlist=model_allowlist or frozenset(),
            model_blocklist=model_blocklist or frozenset(),
        )
        return AccessController(config)

    def test_empty_write_allowlist_allows_all_writes(self):
        """When write_allowlist is empty, all write operations are allowed (subject to Odoo ACL)."""
        controller = self._make_controller(write_allowlist=frozenset())
        mock_conn = MagicMock()
        mock_conn.check_access_rights.return_value = True
        controller.connection = mock_conn

        # Should pass policy check and proceed to Odoo ACL check
        allowed, msg = controller.check_operation_allowed("res.partner", "write")
        assert allowed is True

        allowed, msg = controller.check_operation_allowed("sale.order", "create")
        assert allowed is True

        allowed, msg = controller.check_operation_allowed("res.users", "unlink")
        assert allowed is True
    ...
    def test_write_allowlist_blocks_unlisted_model_writes(self):
        ...
        # res.partner is in write_allowlist — allowed
        allowed, msg = controller.check_operation_allowed("res.partner", "write")
        assert allowed is True

        # account.move is NOT in write_allowlist — blocked
        allowed, msg = controller.check_operation_allowed("account.move", "write")
        assert allowed is False
        assert "WRITE_ALLOWLIST" in msg

        allowed, msg = controller.check_operation_allowed("account.move", "create")
        assert allowed is False
        assert "WRITE_ALLOWLIST" in msg

        allowed, msg = controller.check_operation_allowed("account.move", "unlink")
        assert allowed is False
        assert "WRITE_ALLOWLIST" in msg
    ...
    def test_write_allowlist_does_not_block_reads(self):
        ...
        # account.move is NOT in write_allowlist, but read should still be allowed
    ...
    def test_write_allowlist_with_model_allowlist(self):
        """write_allowlist works together with model_allowlist."""
        controller = self._make_controller(
            model_allowlist=frozenset({"res.partner", "sale.order", "account.move"}),
        ...
        # res.partner: in both lists — write allowed
        allowed, _ = controller.check_operation_allowed("res.partner", "write")
        assert allowed is True

        # sale.order: in model_allowlist but not write_allowlist — write blocked
        allowed, msg = controller.check_operation_allowed("sale.order", "write")
        assert allowed is False
        assert "WRITE_ALLOWLIST" in msg

        # sale.order: read still allowed
        allowed, _ = controller.check_operation_allowed("sale.order", "read")
        assert allowed is True
