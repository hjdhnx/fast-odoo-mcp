...
from fast_odoo_mcp.access_control import AccessControlError, AccessController
from fast_odoo_mcp.config import OdooConfig
from fast_odoo_mcp.error_handling import (
    ValidationError,
)
from fast_odoo_mcp.odoo_connection import OdooConnection, OdooConnectionError
from fast_odoo_mcp.tools import OdooToolHandler
...
class TestOdooToolHandler:
    ...
    @pytest.mark.asyncio
    async def test_get_public_config_returns_safe_runtime_config(
        self, handler, mock_app, valid_config
    ):
        """Test get_public_config returns safe runtime configuration."""
        get_public_config = mock_app._tools["get_public_config"]

        result = await get_public_config()

        assert result.default_limit == valid_config.default_limit
        assert result.max_limit == valid_config.max_limit
        assert result.readonly == valid_config.readonly
        assert result.transport == valid_config.transport
        assert result.api_version == valid_config.api_version
        assert result.odoo_url == valid_config.url
        assert result.max_bulk_size == valid_config.max_bulk_size
        assert result.max_smart_fields == valid_config.max_smart_fields
        assert result.strict_security == valid_config.strict_security
        assert result.stateless_http == valid_config.stateless_http
        assert not hasattr(result, "api_key")
        assert not hasattr(result, "password")
        assert not hasattr(result, "username")
        assert not hasattr(result, "database")
        assert not hasattr(result, "http_token")

    @pytest.mark.asyncio
    async def test_get_public_config_sanitizes_odoo_url(
        self, mock_app, mock_connection, mock_access_controller
    ):
        """Test public config does not expose credentials or URL parameters."""
        config = OdooConfig(
            url="https://user:secret@example.com:8069/odoo?token=abc#frag",
            api_key="test_api_key",
            readonly=False,
        )
        OdooToolHandler(mock_app, mock_connection, mock_access_controller, config)

        result = await mock_app._tools["get_public_config"]()

        assert result.odoo_url == "https://example.com:8069/odoo"
        assert "user" not in result.odoo_url
        assert "secret" not in result.odoo_url
        assert "token" not in result.odoo_url
        assert "frag" not in result.odoo_url
    ...
    async def test_list_models_success(
        ...
        from fast_odoo_mcp.access_control import ModelPermissions
    ...
    async def test_list_models_with_permission_failures(
        ...
        # Setup mocks for get_model_permissions
        from fast_odoo_mcp.access_control import AccessControlError, ModelPermissions
    ...
    async def test_search_records_limit_validation(
        ...
        # Should be capped at max_limit (SearchResult is a Pydantic model)
