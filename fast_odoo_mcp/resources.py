...
class OdooResourceHandler:
    ...
    async def _handle_record_retrieval(self, model: str, record_id: str) -> str:
        ...
        try:
            ...
            record_ids = await self._odoo_call(
                "resource_get_record_search",
                lambda: connection.search(model, [("id", "=", record_id_int)]),
            )
            ...
            try:
                fields_info = await self._odoo_call(
                    "resource_fields_get",
                ...
                if safe_fields:
                    records = await self._odoo_call(
                        "resource_get_record_read_safe",
                        lambda: connection.read(model, record_ids, safe_fields),
                    )
                else:
                    # Fallback to all fields if we can't determine safe fields
                    records = await self._odoo_call(
                        "resource_get_record_read_all",
                        lambda: connection.read(model, record_ids),
                    )
            except Exception as e:
                logger.debug(f"Could not get field metadata, reading all fields: {e}")
                # If we can't get field info, try to read all fields
                records = await self._odoo_call(
                    "resource_get_record_read_fallback",
                    lambda: connection.read(model, record_ids),
                )
        ...
        try:
            ...
            if total_count > 0:
                ...
                if limit_value is not None:
                    search_kwargs["limit"] = limit_value
                if offset_value is not None:
                    search_kwargs["offset"] = offset_value
                if order_value:
                    search_kwargs["order"] = order_value
            ...
            try:
                fields_metadata = await self._odoo_call(
                    "resource_fields_metadata",
                    lambda: connection.fields_get(model),
                )
            except Exception as e:
    ...
    def _parse_domain(self, domain: Optional[str]) -> List[Any]:
        ...
        try:
            ...
            raise ValidationError(
                f"Invalid domain parameter: expected URL-encoded JSON list ({e})"
            ) from e
    ...
    async def _handle_browse(self, model: str, ids: str) -> str:
        ...
        try:
            ...
            try:
                ...
                if safe_fields:
                    records = await self._odoo_call(
                        "resource_browse_read_safe",
                        lambda: connection.read(model, id_list, safe_fields),
                    )
                else:
                    records = await self._odoo_call(
                        "resource_browse_read_all",
                        lambda: connection.read(model, id_list),
                    )
            except Exception as e:
                logger.debug(f"Could not get field metadata, reading all fields: {e}")
                records = await self._odoo_call(
                    "resource_browse_read_fallback",
                    lambda: connection.read(model, id_list),
                )
