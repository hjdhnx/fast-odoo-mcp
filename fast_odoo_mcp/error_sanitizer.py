...
class ErrorSanitizer:
    ...
    def sanitize_message(cls, message: str) -> str:
        ...
        Returns:
            Sanitized error message safe for user consumption, or the original
            message if ODOO_MCP_DEBUG_ERRORS is enabled.
        """
        if not message:
            return "An error occurred"

        # Allow bypassing sanitization in development/debug environments
        if os.getenv("ODOO_MCP_DEBUG_ERRORS", "").lower() in ("1", "true", "yes"):
            return message
