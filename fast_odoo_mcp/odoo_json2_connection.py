...
class OdooJSON2Connection:
    ...
    def execute_kw(
        ...
        ``object.execute_kw`` RPC endpoint.
    ...
    def check_access_rights(self, model: str, operation: str) -> bool:
        ...
        Returns:
            True if access is granted, False if explicitly denied

        Raises:
            OdooConnectionError: If the connection fails (timeout, auth, etc.)
        """
        try:
            result = self._call(
                model,
            ...
            if "Not found" in str(e):
                logger.warning(
                    "check_access_rights not available for %s; denying by default", model
                )
                return False
            raise
