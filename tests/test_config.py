...
from fast_odoo_mcp.config import OdooConfig, get_config, load_config, reset_config, set_config
...
class TestSecurityConfig:
    """Test production safety configuration."""

    def test_security_defaults_are_conservative(self):
        config = OdooConfig(url="http://localhost:8069", api_key="test-key")

        assert config.readonly is True
        assert config.max_bulk_size == 100
        assert config.strict_security is True
        assert config.model_allowlist == frozenset()
        assert config.model_blocklist == frozenset()
        assert config.write_allowlist == frozenset()

    def test_load_security_config_from_env(self, monkeypatch):
        monkeypatch.setenv("ODOO_URL", "http://localhost:8069")
        monkeypatch.setenv("ODOO_API_KEY", "test-key")
        monkeypatch.setenv("ODOO_MCP_READONLY", "false")
        monkeypatch.setenv("ODOO_MCP_MODEL_ALLOWLIST", "res.partner, sale.order")
        monkeypatch.setenv("ODOO_MCP_MODEL_BLOCKLIST", "res.users")
        monkeypatch.setenv("ODOO_MCP_WRITE_ALLOWLIST", "res.partner,sale.order")
        monkeypatch.setenv("ODOO_MCP_MAX_BULK_SIZE", "25")
        monkeypatch.setenv("ODOO_MCP_HTTP_TOKEN", "secret")
        monkeypatch.setenv("ODOO_MCP_MAX_WORKERS", "7")
        monkeypatch.setenv("ODOO_MCP_ALLOWED_HOSTS", "192.168.1.100:8000,192.168.1.100:8001")
        monkeypatch.setenv("ODOO_MCP_ALLOWED_ORIGINS", "http://192.168.1.100:8000")

        config = load_config()

        assert config.readonly is False
        assert config.model_allowlist == frozenset({"res.partner", "sale.order"})
        assert config.model_blocklist == frozenset({"res.users"})
        assert config.write_allowlist == frozenset({"res.partner", "sale.order"})
        assert config.max_bulk_size == 25
        assert config.http_token == "secret"
        assert config.max_workers == 7
        assert config.allowed_hosts == frozenset({"192.168.1.100:8000", "192.168.1.100:8001"})
        assert config.allowed_origins == frozenset({"http://192.168.1.100:8000"})

    def test_public_http_requires_token_in_strict_mode(self):
        with pytest.raises(ValueError, match="ODOO_MCP_HTTP_TOKEN is required"):
            OdooConfig(
    ...
    def test_public_http_allowed_with_token(self):
        config = OdooConfig(
            url="http://localhost:8069",
            api_key="test-key",
            transport="streamable-http",
